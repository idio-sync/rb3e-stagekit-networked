...
try:
    ...
    print("littlefs-python not available - WiFi config UF2 generation will require manual install")


# =============================================================================
# WIFI CONFIG UF2 GENERATOR
# =============================================================================
...
UF2_BOARD_CONFIG = {
    'pico_w': {
        ...
        'display_name': 'Pico W (RP2040)',
    },
    'pico2_w': {
        'family_id': UF2_FAMILY_RP2350_ARM,
        'flash_base': 0x10000000,
        'display_name': 'Pico 2 W (RP2350)',
    },
}
...
def create_littlefs_image(settings_content: bytes) -> bytes:
    """Create a LittleFS image with the settings.toml file."""
...
def generate_wifi_config_uf2(ssid: str, password: str, board: str = 'pico_w') -> bytes:
    """Generate a complete WiFi config UF2 file."""
    if board not in UF2_BOARD_CONFIG:
        raise ValueError(f"Unknown board: {board}")

    # Create settings.toml content
    settings = create_settings_toml(ssid, password)

    # Create LittleFS image
    lfs_image = create_littlefs_image(settings)

    # Get board configuration
    board_cfg = UF2_BOARD_CONFIG[board]
    ...
    # Convert to UF2
    return create_uf2(lfs_image, lfs_address, family_id)
...
class RB3Dashboard:
    ...
    def create_stagekit_status_tab(self, parent):
        ...
        ttk.Button(config_frame, text="Generate WiFi Credentials File",
                   command=self.show_wifi_config_dialog).pack(side='left')

        ttk.Label(config_frame, text="Create a UF2 file to configure WiFi on a new Pico",
                 foreground='gray', font=('TkDefaultFont', 9)).pack(side='left', padx=10)

    def show_wifi_config_dialog(self):
        """Show dialog to generate WiFi credentials UF2 file"""
        dialog = tk.Toplevel(self.root)
        dialog.title("Generate WiFi Credentials File")
        dialog.geometry("450x320")
        dialog.transient(self.root)
        dialog.resizable(False, False)

        # Apply dark styling if available
        ...
        # Title
        ttk.Label(main_frame, text="WiFi Credentials for Stage Kit Pico",
                 font=('TkDefaultFont', 11, 'bold')).pack(pady=(0, 15))

        # SSID field
        ssid_frame = ttk.Frame(main_frame)
        ssid_frame.pack(fill='x', pady=5)
        ttk.Label(ssid_frame, text="WiFi Network (SSID):", width=20, anchor='e').pack(side='left')
        ssid_var = tk.StringVar()
        ssid_entry = ttk.Entry(ssid_frame, textvariable=ssid_var, width=30)
        ssid_entry.pack(side='left', padx=(10, 0))

        # Password field
        pass_frame = ttk.Frame(main_frame)
        pass_frame.pack(fill='x', pady=5)
        ttk.Label(pass_frame, text="WiFi Password:", width=20, anchor='e').pack(side='left')
        pass_var = tk.StringVar()
        pass_entry = ttk.Entry(pass_frame, textvariable=pass_var, width=30, show='*')
        ...
        ttk.Checkbutton(main_frame, text="Show password", variable=show_pass_var,
                       command=toggle_password).pack(pady=5)

        # Board selection
        board_frame = ttk.Frame(main_frame)
        board_frame.pack(fill='x', pady=10)
        ttk.Label(board_frame, text="Target Board:", width=20, anchor='e').pack(side='left')
        board_var = tk.StringVar(value='pico_w')
        board_combo = ttk.Combobox(board_frame, textvariable=board_var, width=27, state='readonly')
        board_combo['values'] = ['pico_w', 'pico2_w']
        board_combo.pack(side='left', padx=(10, 0))

        # Board description
        board_desc = ttk.Label(main_frame, text="Pico W (RP2040)", foreground='gray')
        board_desc.pack()

        def update_board_desc(*args):
            board = board_var.get()
            if board in UF2_BOARD_CONFIG:
                board_desc.config(text=UF2_BOARD_CONFIG[board]['display_name'])
        board_var.trace_add('write', update_board_desc)

        # Instructions link
        link_frame = ttk.Frame(main_frame)
        link_frame.pack(pady=10)
        link_label = ttk.Label(link_frame, text="View setup instructions on GitHub",
                              foreground='#4da6ff', cursor='hand2', font=('TkDefaultFont', 9, 'underline'))
        link_label.pack()
        link_label.bind('<Button-1>', lambda e: webbrowser.open(
            'https://github.com/idio-sync/rb3e-stagekit-networked#firmware-installation'))
        ...
        def create_file():
            ...
            if not ssid:
                messagebox.showwarning("Missing SSID", "Please enter a WiFi network name (SSID).",
                                       parent=dialog)
                return

            if len(ssid) > 32:
                messagebox.showwarning("SSID Too Long", "SSID must be 32 characters or less.",
                                       parent=dialog)
                return

            if len(password) > 64:
                messagebox.showwarning("Password Too Long", "Password must be 64 characters or less.",
                                       parent=dialog)
                return
            ...
            if not LITTLEFS_AVAILABLE:
                result = messagebox.askyesno(
                    "Package Required",
                    "The 'littlefs-python' package is required to generate the UF2 file.\n\n"
                    "Would you like to install it now?\n\n"
                    "(This will run: pip install littlefs-python)",
                    parent=dialog
                )
                if result:
                    try:
                        subprocess.check_call([sys.executable, "-m", "pip", "install", "littlefs-python"])
                        messagebox.showinfo("Installed", "Package installed! Please try again.",
                                            parent=dialog)
                    except Exception as e:
                        messagebox.showerror("Install Failed", f"Failed to install package:\n{e}",
                                             parent=dialog)
                return
            ...
            filepath = filedialog.asksaveasfilename(
                parent=dialog,
                title="Save WiFi Config UF2",
                defaultextension=".uf2",
                initialfile=f"wifi_config_{board}.uf2",
            ...
            try:
                uf2_data = generate_wifi_config_uf2(ssid, password, board)
                with open(filepath, 'wb') as f:
                    f.write(uf2_data)

                messagebox.showinfo(
                    "Success",
                    f"WiFi config UF2 saved to:\n{filepath}\n\n"
                    f"To use:\n"
                    f"1. Hold BOOTSEL and plug in the Pico\n"
                    f"2. Drag this file onto the RPI-RP2 drive",
                    parent=dialog
                )
                dialog.destroy()

            except Exception as e:
                messagebox.showerror("Error", f"Failed to generate UF2:\n{e}", parent=dialog)

        ttk.Button(btn_frame, text="Create", command=create_file, width=12).pack(side='left', padx=5)
